...
message={
    ...
    'signin_fail':'Wrong username or password. Please enter correct username and password',
}
...
def signin(request):
    if request.method == 'POST':
        ...
        if user:
            login(request,user)
            messages.success(request,message['signin_successfull'])
            return redirect("dashboard")
        else:
            messages.warning(request,message['signin_fail'])
        ...
        if password == confirm_password:
            if check_password(password, current_user.password):
                ...
                # seekerprofile
                    ...
                    print(current_user)
                    print(current_user.SeekerProfile)
                    print(current_user.SeekerProfile.qualification)
...
def createdJobbyRecruiter(request):
    ...
    myJob=addjob.objects.filter(created_by=current_user)
    ...
    return render(request,"createdJobbyRecruiter.html",context)

def applicantlist(request,myid):
    
    myjob=addjob.objects.get(id=myid)
    
    applicants=applyjobmodel.objects.filter(apply_to=myjob)
    
    
    myDict={
        'applicants':applicants,
    }
    return render(request,'applicantlist.html',myDict)
...
def applyjob(request,myid):
    current_job=get_object_or_404(addjob,id=myid)
    ...
    if request.method == 'POST':
        ...
        if resume and skills:
            current_user=request.user
            apply=applyjobmodel.objects.create(
                applied_by=current_user,
                apply_to=current_job,
                skills=skills,
                resume=resume,
            )
            apply.save()
