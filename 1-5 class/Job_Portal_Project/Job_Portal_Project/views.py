...
def signup(request):
    ...
    context={
        'signup_success_message':'Account created successfull',
        'password_error_message':'password does not matched',
    }
    ...
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request,context['Account created successfull'])
            return redirect("signin")
        else:
            messages.warning(request,context['password_error_message'])
...
def joblist(request):
    current_user=request.user
    job=addjob.objects.filter(created_by=current_user)
    myDict={
        'mod':job
    }
