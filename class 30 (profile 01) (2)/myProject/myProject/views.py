...
def profile(request):
    ...
    if currentuser.usertype == 'requiter':
        profile=recruiterprofile.objects.get(myuser=currentuser)
    elif currentuser.usertype == 'jobseeker':
        profile=seekerprofile.objects.get(myuser=currentuser)
...
def editprofile(request,myid):
    user=request.user
    if user.usertype == 'requiter':
        profile=recruiterprofile.objects.filter(id=myid)
    elif user.usertype == 'jobseeker':
        profile=seekerprofile.objects.filter(id=myid)
    
    myDct={
        'profile':profile
    }
    return render(request,'editprofile.html',myDct)
