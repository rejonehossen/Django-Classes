...
def deletedoctor(request,myid):
    doctor=doctormodel.objects.get(id=myid)
    doctor.delete()
    return redirect("doctortable")


def deletetable4(request,myid):
    table=table4model.objects.get(id=myid)
    table.delete()
    return redirect("table4")

def deletetable5(request, myid):
    table=table5model.objects.get(id=myid)
    table.delete()
    return redirect("table5")

def viwstable5(request, myid):
    table=table5model.objects.get(id=myid)
